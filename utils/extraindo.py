...
def main(list_input_files=None, references_dict=None, name_output_file="ArquivosExtraidos.xlsx",pages_to_extract=None, max_pages=None, start_page=0):
    ...
    extracted_data = extract_references_from_pdfs(list_input_files, references_dict, start_page=start_page, max_pages=max_pages, pages_to_extract=pages_to_extract)
    ...
    save_table_to_file(df, name_output_file)
    # logging.info(text)
    
# if __name__ == "__main__":
#     main(list_input_files=["fatura.pdf"], references_dict={
#         # "Conta Contrato": r"CONTA CONTRATO:\s*(\d+)",
#         # "Hash_Code": r"Hash Code:\s+([\w\d\.]+)",
#         # "Documento": r"Documento:\s+(\d+)",
#         # "Empresa": r"Empresa:\s+([^\n]+)",
...
#         # "V.F.: Tributo a Reter IRPJ (Valor) ": r"Tributo a Reter IRPJ\s+([\d\.,]+)",
#         # "V.F.: Saldo em Aberto (Valor)": r"Saldo em aberto\s+([\d\.,]+)",
#     }, start_page=1, max_pages=2, pages_to_extract=None)